...
export function usePromptInputProvider(props: {
  ...
  const matchesAccept = (file: File) => {
    if (props.accept === undefined || props.accept.trim() === "") return true;
  ...
  const addFiles = (incoming: File[] | FileList) => {
    ...
    if (fileList.length > 0 && accepted.length === 0) {
    ...
    const withinSize = (file: File) =>
      props.maxFileSize === undefined || file.size <= props.maxFileSize;
    ...
    const capacity =
      props.maxFiles === undefined ? undefined : Math.max(0, props.maxFiles - currentCount);
